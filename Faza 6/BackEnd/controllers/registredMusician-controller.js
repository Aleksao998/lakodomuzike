...
exports.createRegistredMusician = asyncHandler(async (req, res, next) => {
  ...
  const musician = await Musician.findOne({ user: req.user.id });
  const ad = await Ad.findOne({ user: req.user.id });

  if (!musician) {
    new ErrorResponse(`Musician not found with id of ${req.params.id}`, 404);
  }
  if (!ad) {
...
exports.updateRegistredMusician = asyncHandler(async (req, res, next) => {
  ...
  if (
    ...
    return next(
      new ErrorResponse(
        `RegistratedMusician id ${req.params.id} is not authorized to update`,
        401
      )
    );
  }

  registredmusician = await RegistredMusician.findByIdAndUpdate(
    req.params.id,
    req.body,
    {
      new: true,
